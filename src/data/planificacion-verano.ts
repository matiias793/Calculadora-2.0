...
const generarPlanificacion = (): DiaPlanificacion[] => {
    const planificacion: DiaPlanificacion[] = [];
    ...
    while (fechaActual <= fechaFin) {
        ...
        if (diaSemana !== 0 && diaSemana !== 6) {
            // Función simple de hash para seed
            ...
            let attempts = 0;
            do {
                // Usar fecha como seed para determinismo
                const seed = fechaActual.getTime() + attempts;
            ...
            const seedDesayuno = fechaActual.getTime() + 100;
...
export const PLANIFICACION_VERANO = generarPlanificacion();

export const getPlanificacionPorFecha = (fechaIso: string) => {
    return PLANIFICACION_VERANO.find(p => p.fecha === fechaIso);
};
